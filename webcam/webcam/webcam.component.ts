import { AfterViewInit, Component, EventEmitter, Input, OnDestroy, Output, OnInit, ViewChild } from '@angular/core';
import { WebcamInitError } from "../domain/webcam-init-error";
import { WebcamImage } from "../domain/webcam-image";
import { Observable, Subscription } from "rxjs";
import { WebcamUsed } from "../webcamused/webcam.used";
...
export class WebcamComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    @Output() mirorcanvas = new EventEmitter();
    ...
    miroir_viedo = { active: true };

    constructor() {
        this.miroir_viedo.active = JSON.parse(localStorage.getItem('prepare'))
    }
    ...
    public ngAfterViewInit(): void {
        ...
        this.detectAvailableDevices()
            .then((devices: MediaDeviceInfo[]) => {
                // start first device
                this.switchToVideoInput(devices.length > 0 ? devices[0].label : '');
            })
            .catch((err: string) => {
    ...
    this.miroir_viedo.active = !this.miroir_viedo.active
        return sessionStorage.setItem("prepare", JSON.stringify(this.miroir_viedo.active))
    }
     ...
     */
    private static getMediaConstraintsForDevice(deviceId: any, baseMediaTrackConstraints: MediaTrackConstraints) {
        let result: MediaTrackConstraints = baseMediaTrackConstraints ? baseMediaTrackConstraints : this.DEFAULT_VIDEO_OPTIONS;
        for (var i = 0; i !== deviceId.length; ++i){
            var deviceInfo = deviceId[i];
            if (deviceInfo.kind === 'videoinput'){
                result.deviceId= {exact:deviceInfo.kind}
            }
        }
    }
     ...
     */
    public switchToVideoInput(label: string): void {
        this.videoInitialized = false;
        this.stopMediaTracks();
        this.initWebcam(label, this.videoOptions);
    }
        ...
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            ...
            navigator.mediaDevices.getUserMedia(<MediaStreamConstraints>{ video: true })
                .then((stream: MediaStream) => {
                    ...
                    let activeDeviceId = WebcamComponent.getDeviceIdFromMediaStreamTrack(stream.getVideoTracks()[0]);
                
                    this.activeVideoInputIndex = activeDeviceId ? this.availableVideoInputs
                        .findIndex((mediaDeviceInfo: MediaDeviceInfo) => mediaDeviceInfo.label === activeDeviceId, ) : -1;
    ...
    private detectAvailableDevices(): Promise<MediaDeviceInfo[]> {
        return new Promise((resolve, reject) => {
            WebcamUsed.getAvailableVideoInputs()
                .then((devices: MediaDeviceInfo[]) => {
